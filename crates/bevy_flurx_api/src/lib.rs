...
mod tests {
    ...
    pub fn test_app() -> App {
        let mut app = App::new();
        app.add_plugins((MinimalPlugins, FlurxIpcPlugin));
        app
    }
    ...
    pub fn assert_api_registered<P: Plugin>(plugin: P, ipc_id: &'static str) {
        ...
        app.add_systems(Startup, |mut commands: Commands| {
            commands.spawn((IpcHandlers::default(), WebviewInitialized(())));
        });
        app.update();
        app.world_mut()
            .run_system_once(move |handlers: Query<&IpcHandlers>| {
                assert!(
                    handlers
                        .single()
